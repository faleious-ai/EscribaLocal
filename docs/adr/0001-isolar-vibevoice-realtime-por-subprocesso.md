...
- Para possibilitar uma transição segura e o diagnóstico automatizado de dependências do checkpoint local, o worker oferece uma rotina controlada de deep probe via variáveis de ambiente. Essa rotina investiga a carga dos componentes de forma sequencial (imports, config, processor, tokenizer, feature extractor e classes de modelagem do HuggingFace) sem expor a pilha de execução ao processo principal, gerando telemetria detalhada sobre eventuais pontos de falha.
